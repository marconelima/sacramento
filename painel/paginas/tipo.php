...
if(isset($_POST['btn_status']) && $_POST['btn_status'] == "Desativar"){
	$sql_desativar = "UPDATE tbtipo SET status = 0 WHERE id = ".$_POST['item_id'];
	$conecta->selecionar($conecta->conn,$sql_desativar);
} elseif(isset($_POST['btn_status']) && $_POST['btn_status'] == "Ativar") {
	$sql_ativar = "UPDATE tbtipo SET status = 1 WHERE id = ".$_POST['item_id'];
...
if(isset($_POST['gravar']) && $_POST['gravar'] == "Gravar"){
	...
	//PEGAR OS DADOS
	$dados = $_POST["dados"];
	$id = 			strip_tags(trim($dados['tbtipo']['id']));
	$titulo =    	strip_tags(trim($dados['tbtipo']['titulo']));
	...
	if(!$rs_paicategoria = mysqli_fetch_array($resultado_filhocategoria)){
		$resultado = $conecta->selecionar($conecta->conn,"DELETE FROM tbtipo WHERE id = $id");
	} else {
		$resultado = 0;
	}
	if($resultado == 1){
	...
	$id = $_GET['alterar'];
	$resultado = $conecta->selecionar($conecta->conn,"SELECT * FROM tbtipo WHERE id = $id");
...
?>
    ...
    <a href="home.php?pagina=tipo&amp;tela=<?php echo $idtela; ?>&amp;vw=1"><button type="button" class="btn btn-primary btn_direita"><span class="glyphicon glyphicon-plus"></span> Inserir <?php echo $nometela;?></button></a>
	...
	<form class="form-horizontal" role="form" name="formNoticia" method="post" enctype="multipart/form-data" action="">
    	<input type="hidden" name="dados[tbtipo][id]" value="<?php echo @$id; ?>" />
        <input type="hidden" name="dados[tbtipo][tela_id]" value="<?php echo @$idtela; ?>" />

    	<div class="form-group">
            <label for="titulo" class="col-sm-2 control-label">Título</label>
            <div class="col-sm-10">
              <input type="text" name="dados[tbtipo][titulo]" value="<?php echo @$titulo; ?>" class="form-control" id="titulo" placeholder="Título">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
              <div class="input-group">
                  <input type="radio" id="radio" name="dados[tbtipo][status]" <?php if(@$status == 1 ) { echo "checked"; } ?> value="1"> Sim
                  <input type="radio" id="radio" name="dados[tbtipo][status]" <?php if(@$status == 2 ) { echo "checked"; } ?> value="2"> Não
               </div>
    ...
    <?php } else {?>
        ...
        <table class="table table-hover">
          ...
          <tbody>
    	<?php
		$sql = "SELECT * FROM tbtipo WHERE tela_id = $idtela"." ORDER BY titulo ASC";
	  ...
	  ?>
		<tr>
		  ...
		  <td align="center">
		  <form action="home.php?pagina=tipo&amp;tela=<?php echo $tela_id;?>" method="post" enctype="multipart/form-data" name="form_ativa">
		  ...
		  <td align="center"><a href="home.php?pagina=tipo&amp;vw=1&amp;tela=<?php echo $idtela;?>&amp;alterar=<?php echo $rs['id']?>&amp;p=<?php echo $p;?>" ><span class="glyphicon glyphicon-pencil"></span></a></td>
		  <td align="center"><a href="home.php?pagina=tipo&amp;tela=<?php echo $idtela;?>&amp;apagar=<?php echo $rs['id']?>&amp;p=<?php echo $p;?>" ><span class="glyphicon glyphicon-remove"></span></a></td>
		</tr>
		<?php } ?>
	  </tbody>
      </table>
