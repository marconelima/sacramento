...
if(isset($_POST['btn_status']) && $_POST['btn_status'] == "Desativar"){
	$sql_desativar = "UPDATE tbmontadora SET status = 0 WHERE id = ".$_POST['item_id'];
	$conecta->selecionar($conecta->conn,$sql_desativar);
} elseif(isset($_POST['btn_status']) && $_POST['btn_status'] == "Ativar") {
	$sql_ativar = "UPDATE tbmontadora SET status = 1 WHERE id = ".$_POST['item_id'];
...
if(isset($_POST['gravar']) && $_POST['gravar'] == "Gravar"){
	...
	//PEGAR OS DADOS
	$dados = $_POST["dados"];
	$id = 			strip_tags(trim($dados['tbmontadora']['id']));
	$titulo =    	strip_tags(trim($dados['tbmontadora']['titulo']));
	...
	$id = $_GET['apagar'];
	$sql_filhocategoria = "SELECT id FROM tbmodelo WHERE montadora_id = $id";
	...
	if(!$rs_paicategoria = mysqli_fetch_array($resultado_filhocategoria)){
		$resultado = $conecta->selecionar($conecta->conn,"DELETE FROM tbmontadora WHERE id = $id");
	} else {
		$resultado = 0;
	}
	if($resultado == 1){
	...
	$id = $_GET['alterar'];
	$resultado = $conecta->selecionar($conecta->conn,"SELECT * FROM tbmontadora WHERE id = $id");
...
?>
    ...
    <a href="home.php?pagina=montadora&amp;tela=<?php echo $idtela; ?>&amp;vw=1"><button type="button" class="btn btn-primary btn_direita"><span class="glyphicon glyphicon-plus"></span> Inserir Montadoras</button></a>
	...
	<form class="form-horizontal" role="form" name="formNoticia" method="post" enctype="multipart/form-data" action="">
    	<input type="hidden" name="dados[tbmontadora][id]" value="<?php echo @$id; ?>" />
        <input type="hidden" name="dados[tbmontadora][tela_id]" value="<?php echo @$idtela; ?>" />

    	<div class="form-group">
            <label for="titulo" class="col-sm-2 control-label">Título</label>
            <div class="col-sm-10">
              <input type="text" name="dados[tbmontadora][titulo]" value="<?php echo @$titulo; ?>" class="form-control" id="titulo" placeholder="Título">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
              <div class="input-group">
                  <input type="radio" id="radio" name="dados[tbmontadora][status]" <?php if(@$status == 1 ) { echo "checked"; } ?> value="1"> Sim
                  <input type="radio" id="radio" name="dados[tbmontadora][status]" <?php if(@$status == 2 ) { echo "checked"; } ?> value="2"> Não
               </div>
    ...
    <?php } else {?>
        ...
        <table class="table table-hover">
          ...
          <tbody>
    	<?php
		$sql = "SELECT * FROM tbmontadora WHERE tela_id = $idtela ORDER BY titulo ASC";
	  ...
	  ?>
		<tr>
		  ...
		  <td align="center">
		  <form action="home.php?pagina=montadora&amp;tela=<?php echo $tela_id;?>" method="post" enctype="multipart/form-data" name="form_ativa">
		  ...
		  <td align="center"><a href="home.php?pagina=montadora&amp;vw=1&amp;tela=<?php echo $idtela;?>&amp;alterar=<?php echo $rs['id']?>&amp;p=<?php echo $p;?>" ><span class="glyphicon glyphicon-pencil"></span></a></td>
		  <td align="center"><a href="home.php?pagina=montadora&amp;tela=<?php echo $idtela;?>&amp;apagar=<?php echo $rs['id']?>&amp;p=<?php echo $p;?>" ><span class="glyphicon glyphicon-remove"></span></a></td>
		</tr>
		<?php } ?>
	  </tbody>
      </table>
