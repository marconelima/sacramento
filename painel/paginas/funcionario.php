...
if(isset($_POST['gravar']) && $_POST['gravar'] == "Gravar"){
	...
	//PEGAR OS DADOS
	$dados = $_POST["dados"];
	$id = 			strip_tags(trim($dados['tbfuncionario']['id']));
	$titulo =    	strip_tags(trim($dados['tbfuncionario']['titulo']));
	$cargo =    	strip_tags(trim($dados['tbfuncionario']['cargo']));
	$status =    	strip_tags(trim($dados['tbfuncionario']['status']));
	...
	if (empty($retorno)) {
		...
		$dados['tbfuncionario']['arquivo'] = @$nome;
		}
	...
	$sql_busca = "SELECT arquivo FROM tbfuncionario WHERE id = $id";
	...
	$resultado = $conecta->selecionar($conecta->conn,"DELETE FROM tbfuncionario WHERE id = $id");
	if($resultado == 1){
	...
	$id = $_GET['alterar'];
	$resultado = $conecta->selecionar($conecta->conn,"SELECT * FROM tbfuncionario WHERE id = $id");
	$rs = mysqli_fetch_array($resultado);
	$id = $rs['id'];
	$arquivo = $rs['arquivo'];
	$titulo = $rs['titulo'];
...
?>
	<a href="home.php?pagina=funcionario&amp;vw=1&amp;tela=<?php echo $_GET['tela'];?>" ><button type="button" class="btn btn-primary btn_direita"><span class="glyphicon glyphicon-plus"></span> Inserir <?php echo $nometela;?></button></a>
	...
	<form class="form-horizontal" role="form"  name="formcategoriaproduto" method="post" enctype="multipart/form-data" action="">
    	<input type="hidden" name="dados[tbfuncionario][id]" value="<?php echo $id; ?>" />
    	<div class="form-group">
            <label for="titulo" class="col-sm-2 control-label">Nome:</label>
            <div class="col-sm-10">
            <input type="text" name="dados[tbfuncionario][titulo]" value="<?php echo @$titulo; ?>" class="form-control" id="titulo" placeholder="Título"/>
             </div>
        </div>
        <div class="form-group">
            <label for="titulo" class="col-sm-2 control-label">Cargo:</label>
            <div class="col-sm-10">
            <input type="text" name="dados[tbfuncionario][cargo]" value="<?php echo @$cargo; ?>" class="form-control" id="cargo" placeholder="Cargo"/>
             </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
              <div class="input-group">
                  <input type="radio" id="radio" name="dados[tbfuncionario][status]" <?php if(@$status == 1 ) { echo "checked"; } ?> value="1"> Sim
                  <input type="radio" id="radio" name="dados[tbfuncionario][status]" <?php if(@$status == 2 ) { echo "checked"; } ?> value="2"> Não
               </div>
    ...
    <?php } else {?>
    	<?php
		$sql = "SELECT * FROM tbfuncionario ORDER BY titulo ASC LIMIT $inicio, $maximo";
		$sql_paginacao = "SELECT * FROM tbfuncionario";
	...
	<div class="separa"></div>
        ...
        <table class="table table-hover">
		  ...
		  <td align="center"><a href="home.php?pagina=funcionario&amp;vw=1&amp;tela=<?php echo $idtela;?>&amp;alterar=<?php echo $rs['id']?>" ><span class="glyphicon glyphicon-pencil"></span></a></td>
		  <td align="center"><a href="home.php?pagina=funcionario&amp;tela=<?php echo $idtela;?>&amp;apagar=<?php echo $rs['id']?>" ><span class="glyphicon glyphicon-remove"></span></a></td>
		</tr>
            <?php } ?>
        </tbody>
      </table>
