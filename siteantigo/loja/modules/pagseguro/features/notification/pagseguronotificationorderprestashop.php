...
class PagSeguroNotificationOrderPrestashop
{
    ...
    private function saveOrder($id_order, $transaction,$installmentcount)
    {
		...
		$this->updateOrderPs($id_order, $installmentcount, $transaction);
    }
	
    private function updateOrderPs($id_order, $installmentcount, $transaction)
    {
        $sql = 'UPDATE `' . _DB_PREFIX_ . 'orders`
        SET `installmentcounts` = \'' . (int) $installmentcount . '\',
        `id_transaction` = \'' . pSQL($transaction) . '\'
   		WHERE `id_order` = \'' . (int) $id_order . '\';';
        if (! Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute($sql)) {
            die(Tools::displayError('Error when updating InstallmentCount from PagSeguro in database'));
        }
    }
