...
<h1><span>{l s='Your addresses'}</span></h1>
...
<form action="{$link->getPageLink('address', true)|escape:'html'}" method="post" class="std titled_box" id="add_address">
	...
	{foreach from=$ordered_adr_fields item=field_name}
		...
		<p class="required postcode form-group">
			...
			<input type="tel" class="form-control" id="postcode" name="postcode" value="{if isset($smarty.post.postcode)}{$smarty.post.postcode}{else}{if isset($address->postcode)}{$address->postcode|escape:'html'}{/if}{/if}" onkeyup="$('#postcode').val($('#postcode').val().toUpperCase());" />
		</p>
		{/if} -->
		...
		*}
		...
		<p class="form-group">
			...
			<input type="tel" id="phone" class="form-control" name="phone" value="{if isset($smarty.post.phone)}{$smarty.post.phone}{else}{if isset($address->phone)}{$address->phone|escape:'html'}{/if}{/if}" />
		</p>
		<p class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
			<label for="phone_mobile">{l s='Mobile phone'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>*</sup>{/if}</label>
			<input type="tel" id="phone_mobile" class="form-control" name="phone_mobile" value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{else}{if isset($address->phone_mobile)}{$address->phone_mobile|escape:'html'}{/if}{/if}" />
		</p>
		<p class="required form-group" id="adress_alias">
			...
			<input type="text" id="alias" class="form-control" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{else if isset($address->alias)}{$address->alias|escape:'html'}{elseif !$select_address}{l s='My address'}{/if}" />
		</p>
        </div>
        </div>    
	</fieldset>
	<p class="submit2">
		...
		<input type="submit" name="submitAddress" id="submitAddress" value="{l s='Save'}" class="button btn btn-default" />
	</p>
</form>
